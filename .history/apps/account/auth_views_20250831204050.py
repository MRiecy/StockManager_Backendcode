    ...
    try:
        ...
        with transaction.atomic():
            ...
            user = User.objects.create_user(
                ...
                email=data.get('phone', ''),  # 将phone作为email存储
                first_name=data.get('nickname', '')  # 将nickname作为first_name存储
            )
...
@api_view(['POST'])
@permission_classes([AllowAny])  # 允许任何人调用，即使token无效
def logout(request):
    """退出登录"""
    try:
        # 尝试获取用户，但不强制要求认证
        ...
        UserToken.objects.filter(user=user).update(is_active=False)
            ...
            # 如果用户未认证，尝试从请求头获取token并标记为无效
            auth_header = request.headers.get('Authorization', '')
            if auth_header.startswith('Bearer '):
                token = auth_header.split(' ')[1]
                # 标记包含此token的记录为无效
                UserToken.objects.filter(access_token=token).update(is_active=False)
                message = "退出登录成功（令牌已失效）"
            else:
                message = "退出登录成功（无需操作）"
