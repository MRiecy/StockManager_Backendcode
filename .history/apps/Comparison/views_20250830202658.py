...
def yearly_comparison(request):
    ...
    try:
        ...
        # 3. 查询历史数据 - 使用正确的参数格式
        try:
            # 使用正确的参数格式：股票代码列表、开始日期列表、结束日期列表
            stock_codes = [stock_code]
            start_dates = ['20220101']
            end_dates = ['20221231']
            
            # 获取2022年数据
            data_2022 = xtdata.get_market_data(stock_codes, start_dates, end_dates, '1d')
            
            # 获取2023年数据
            start_dates_2023 = ['20230101']
            end_dates_2023 = ['20231231']
            data_2023 = xtdata.get_market_data(stock_codes, start_dates_2023, end_dates_2023, '1d')
            
            # 获取2024年数据
            start_dates_2024 = ['20240101']
            end_dates_2024 = ['20241231']
            data_2024 = xtdata.get_market_data(stock_codes, start_dates_2024, end_dates_2024, '1d')
            
            yearly_data = []
            
            # 处理2022年数据
            if data_2022 and len(data_2022) > 0 and len(data_2022[0]) > 0:
                start_price_2022 = data_2022[0][0].get('close', 0)
                end_price_2022 = data_2022[0][-1].get('close', 0)
            ...
            # 处理2023年数据
            if data_2023 and len(data_2023) > 0 and len(data_2023[0]) > 0:
                start_price_2023 = data_2023[0][0].get('close', 0)
                end_price_2023 = data_2023[0][-1].get('close', 0)
            ...
            # 处理2024年数据
            if data_2024 and len(data_2024) > 0 and len(data_2024[0]) > 0:
                start_price_2024 = data_2024[0][0].get('close', 0)
                end_price_2024 = data_2024[0][-1].get('close', 0)
            ...
            if yearly_data:
                return JsonResponse({
                    ...
                    'data_points': len(data_2022[0]) + len(data_2023[0]) + len(data_2024[0]) if data_2022 and data_2023 and data_2024 else 0
                })
            else:
...
def weekly_comparison(request):
    ...
    try:
        ...
        try:
            ...
            # 查询历史数据 - 使用正确的参数格式
            stock_codes = [stock_code]
            start_dates = [start_date_str]
            end_dates = [end_date_str]
            data = xtdata.get_market_data(stock_codes, start_dates, end_dates, '1d')
            ...
            if data and len(data) > 0 and len(data[0]) > 0:
                ...
                # 按周分组数据
                weekly_groups = {}
                for record in data[0]:  # data[0] 是第一个股票的数据
                    if 'time' in record:
