...
class AssetComparisonView(APIView):
    """资产对比数据API - 基于DuckDB+Parquet缓存的高性能方案"""
    ...
    def post(self, request):
        ...
        请求参数:
        {
            "symbols": ["600519.SH", "000001.SZ"],  // 股票代码列表
            "period": "1d",                         // K线周期: 1m, 5m, 15m, 30m, 1h, 1d
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    end_dt = datetime.strptime(end_time, "%Y-%m-%d") if end_time else datetime.now()
                except ValueError:
                    return Response({
                        "code": 400, 
            ...
            for symbol, df in data_dict.items():
                if not df.empty:
                    ...
                    if 'time' in df_copy.columns:
                        # 检查time列的数据类型
                        if df_copy['time'].dtype == 'int64' or df_copy['time'].dtype == 'float64':
                            # 时间戳格式
                            df_copy['time'] = pd.to_datetime(df_copy['time'], unit='s').dt.strftime('%Y-%m-%d %H:%M:%S')
                        else:
                            # 已经是datetime格式
                            df_copy['time'] = pd.to_datetime(df_copy['time']).dt.strftime('%Y-%m-%d %H:%M:%S')
                    
                    result[symbol] = df_copy.to_dict(orient='records')
                else:
                    result[symbol] = []
            ...
            return Response({
                ...
                "meta": {
                    ...
                    "end_time": end_time,
                    "data_points": {symbol: len(data) for symbol, data in result.items()}
                }
            })
            ...
            print(f"AssetComparisonView错误: {error_details}")
            
            return Response({
                "code": 500, 
                "message": f"服务器内部错误: {str(e)}",
                "error_details": error_details if request.data.get('debug') else None
...
class CacheManagementView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            cache_stats = CacheIndex.objects.aggregate(
                total_files=Count('id'),
                total_size_bytes=Sum('size_bytes')
            )
            
            total_size_mb = (cache_stats['total_size_bytes'] or 0) / (1024 * 1024)
            total_size_gb = total_size_mb / 1024
            
            # 按股票代码统计
            symbol_stats = CacheIndex.objects.values('symbol').annotate(
                file_count=Count('id'),
            ...
            return Response({
                "code": 200,
                "data": {
                    "total_files": cache_stats['total_files'] or 0,
                    "total_size_bytes": cache_stats['total_size_bytes'] or 0,
                    "total_size_mb": round(total_size_mb, 2),
                    "total_size_gb": round(total_size_gb, 2),
                    "max_size_gb": getattr(settings, 'MAX_CACHE_SIZE_GB', 2.0),
                    "cache_usage_percent": round((total_size_gb / getattr(settings, 'MAX_CACHE_SIZE_GB', 2.0)) * 100, 2),
    ...
    def delete(self, request):
        """清理缓存"""
        try:
            from .services import enforce_cache_limit
            from .models import CacheIndex
            
            # 获取清理前的统计
            before_count = CacheIndex.objects.count()
            
            # 执行缓存清理
            enforce_cache_limit()
            
            # 获取清理后的统计
            after_count = CacheIndex.objects.count()
            ...
            return Response({
                ...
                "data": {
                    ...
                    "remaining_files": after_count
                },
                "message": f"缓存清理完成，删除了 {cleaned_count} 个文件"
            })
