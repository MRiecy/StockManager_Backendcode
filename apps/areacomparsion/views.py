...
def area_comparison(request):
    try:
        try:
            # 创建交易接口实例
            path = r'D:\迅投极速交易终端 睿智融科版\userdata'  # 使用实际的迅投安装路径
            print(f"尝试使用路径: {path}")
            ...
            if connect_result != 0:
                ...
                print(error_msg)
                return get_mock_area_comparison_data(None)
            ...
            for acc in accounts:
                ...
                if not asset:
                    print(f"未查询到账户 {account_id} 的资产信息，跳过")
                    continue

                # 查询该账户的持仓信息
                positions = xt_trader.query_stock_positions(acc)
                if not positions:
                    print(f"未查询到账户 {account_id} 的持仓信息，跳过")
                    continue
...
def get_mock_area_comparison_data(account_id):
    ...
    # 模拟地区列表
    regions = ['上交所', '深交所', '创业板', '科创板', '北交所', '港股']
    ...
    for i in range(num_accounts):
        ...
        # 添加模拟账户数据
        all_accounts_data.append({
            'account_id': f"mock_account_{i+1}",
            'total_market_value': round(total_market_value, 2),
            'area_data': area_data
        })
